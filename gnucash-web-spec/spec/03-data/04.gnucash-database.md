# 03.04 - GnuCash Database Baseline and Relational Hardening

## Purpose

This document anchors the project to the legacy GnuCash relational schema while allowing explicit quality improvements in implementations.

Guidance:
- The DDL in this file is the baseline structure reference.
- `gnucash-web-spec` SHOULD stay semantically aligned with this baseline where possible.
- Implementations SHOULD add relational integrity constraints (foreign keys, uniqueness, and checks) when they are safe and deterministic.

## Scope note

The current normative MVP scope remains:
- Book
- Commodity
- Account

Hardening requirements in this section prioritize those entities first.

## Relational hardening profile (MVP-first)

For SQL implementations that follow this schema, apply the following improvements.
...
ALTER TABLE public.accounts
  ADD CONSTRAINT accounts_commodity_fk
  FOREIGN KEY (commodity_guid) REFERENCES public.commodities(guid)
  ON UPDATE RESTRICT ON DELETE RESTRICT;

ALTER TABLE public.accounts
  ADD CONSTRAINT accounts_parent_fk
  FOREIGN KEY (parent_guid) REFERENCES public.accounts(guid)
  ON UPDATE RESTRICT ON DELETE RESTRICT;

-- Books root account relationship
ALTER TABLE public.books
  ADD CONSTRAINT books_root_account_fk
  FOREIGN KEY (root_account_guid) REFERENCES public.accounts(guid)
  ON UPDATE RESTRICT ON DELETE RESTRICT;
```

### 2) Uniqueness and check constraints

```sql
-- Commodity identity
ALTER TABLE public.commodities
  ADD CONSTRAINT commodities_namespace_mnemonic_uk
  UNIQUE (namespace, mnemonic);

-- Commodity fraction must be positive
ALTER TABLE public.commodities
  ADD CONSTRAINT commodities_fraction_positive_ck
  CHECK (fraction > 0);

-- Account cannot be its own parent
ALTER TABLE public.accounts
  ADD CONSTRAINT accounts_not_self_parent_ck
  CHECK (parent_guid IS NULL OR parent_guid <> guid);
```

### 3) Migration safety strategy

When applying these constraints to existing data:
- Add constraints as `NOT VALID` first.
- Clean invalid rows.
- Run `VALIDATE CONSTRAINT`.

This reduces migration risk in legacy databases with historical inconsistencies.

## Extended hardening (optional, outside MVP conformance)

Implementations MAY add FKs to additional deterministic relationships (for example `splits -> transactions`, `splits -> accounts`, `prices -> commodities`) as long as they do not break existing valid datasets.

## Legacy DDL snapshot

The content below is the schema snapshot used as baseline.
