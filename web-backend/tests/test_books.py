...
def test_create_book_missing_name(client):
    # BookCreate.name is Optional[str] — API accepts missing name and stores null.
    resp = client.post("/books", json={})
    assert resp.status_code == 201
    assert resp.json()["name"] is None


def test_create_book_null_name(client):
    # Explicit null name is also accepted (same schema: str | None = None).
