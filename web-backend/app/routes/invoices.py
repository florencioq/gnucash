...
def _invoice_due_date(invoice: Invoice, *, due_dates_by_tx_guid: dict[str, datetime]) -> datetime | None:
...
def _invoice_to_out(
    ...
    *,
    ...
    date_due = _invoice_due_date(invoice, due_dates_by_tx_guid=due_date_map)
...
def _invoice_to_list_item(
    ...
    *,
    ...
    return InvoiceListItemOut(
        ...
        date_opened=invoice.date_opened,
        date_posted=invoice.date_posted,
        date_due=_invoice_due_date(invoice, due_dates_by_tx_guid=due_dates_by_tx_guid),
        currency_guid=invoice.currency_guid,
        customer_guid=invoice.owner_guid,
...
def list_invoices(
    ...
    return [_invoice_to_out(db, invoice, due_dates_by_tx_guid=due_dates_by_tx_guid) for invoice in invoices]


@router.get("/source-links", response_model=list[DocumentSourceLinkOut])
def list_invoice_source_links(
    book_id: UUID = Query(...),
    customer_guid: UUID | None = Query(default=None),
    db: Session = Depends(get_db),
) -> list[dict]:
...
def list_invoices_paginated(
    ...
    sort_key: Literal["id", "customer", "date_opened", "date_posted", "date_due", "posted_status", "payment_status", "total", "open"] = Query(default="date_opened"),
    ...
    needs_python_filter_or_sort = (
        ...
        or sort_key in {"customer", "payment_status", "total", "open", "date_due"}
    )
    ...
    if not needs_python_filter_or_sort:
        ...
        items = [
            _invoice_to_list_item(
                db,
                invoice,
                customer_name=customers_by_guid.get(invoice.owner_guid),
